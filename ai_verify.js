...
//
// Usage (from app.js):
// const verifier = await createAIVerifier({ videoEl, canvasEl, statusEl });
// await verifier.start();
// const result = await verifier.captureAndVerify({ template:"tpose", pinName:"..." });
// await verifier.stop();
...
async function ensureMediaPipePoseLoaded() {
  // MediaPipe UMD globals:
  // window.Pose, window.Camera
  if (window.Pose && window.Camera) return;

  // Pose + camera utils from CDN (pinned version)
...
function above(a, b, pad = 0.02) {
  // "a is above b" in normalized coords (y smaller = higher)
  if (!a || !b) return false;
  return a.y < b.y - pad;
}
...
function roughlyStraightArm(shoulder, elbow, wrist, tol = 0.18) {
  // Check elbow is “between” shoulder and wrist-ish (not super bent)
  if (!shoulder || !elbow || !wrist) return false;
  const a = dist(shoulder, elbow);
  const b = dist(elbow, wrist);
  const c = dist(shoulder, wrist);
  // triangle inequality: if arm is straight, a+b ~= c
  return Math.abs(a + b - c) < tol;
}

function visibilityOK(landmarks, minVis = 0.45) {
  // Landmarks have "visibility" on some builds; tolerate if missing
  if (!landmarks) return false;
  const sampleIdx = [11, 12, 13, 14, 15, 16]; // shoulders/elbows/wrists
  for (const i of sampleIdx) {
    const p = landmarks[i];
    if (!p) return false;
    if (typeof p.visibility === "number" && p.visibility < minVis) return false;
  }
  return true;
}
...
function verifyTemplate(landmarks, template) {
  ...
  if (template === "still") {
    // For “still”, we verify just “standing upright-ish”: wrists below shoulders + shoulders above hips
  ...
  if (template === "tpose") {
    // Arms out: wrists roughly same height as shoulders + arms not super bent
  ...
  if (template === "hands_together") {
    // Hands together: wrists near each other at chest-ish height (between shoulders & hips)
    const together = near(LW, RW, 0.08);
    const chestBand =
      LW.y > Math.min(LS.y, RS.y) && LW.y < Math.max(LH.y, RH.y); // between shoulders and hips
...
export async function createAIVerifier({ videoEl, canvasEl, statusEl }) {
  ...
  pose.onResults((res) => {
    ...
    // Optional: draw skeleton dots (simple)
    if (!canvasEl) return;
  ...
  async function captureAndVerify({
    ...
    if (!videoEl) return { ok: false, reason: "No video element" };

    // Create snapshot blob from canvas (preferred) or from video frame
